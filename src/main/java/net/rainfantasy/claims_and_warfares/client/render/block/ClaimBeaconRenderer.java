...
public class ClaimBeaconRenderer implements BlockEntityRenderer<ClaimBeaconBlockEntity> {
	...
	private void renderExtendedInfo(@NotNull ClaimBeaconBlockEntity block, float delta, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight, int packedOverlay){
		...
		if(BeaconHelper.isUnstableState(block.getStatus())){
	...
	private static @NotNull Component getErrorCode(@NotNull ClaimBeaconBlockEntity block) {
		...
		switch (block.getErrorCode()){
			case ClaimBeaconBlockEntity.ERROR_NO_FUEL -> content3 = Component.translatable("caw.label.beacon.error.no_fuel");
			case ClaimBeaconBlockEntity.ERROR_NO_OWNER -> content3 = Component.translatable("caw.label.beacon.error.no_owner");
			case ClaimBeaconBlockEntity.ERROR_CLAIM_CONFLICT -> content3 = Component.translatable("caw.label.beacon.error.claim_conflict");
			case ClaimBeaconBlockEntity.ERROR_OBSTRUCTED -> content3 = Component.translatable("caw.label.beacon.error.obstructed");
			case ClaimBeaconBlockEntity.ERROR_UPGRADE_CHANGED -> content3 = Component.translatable("caw.label.beacon.error.upgrade_changed");
		}
		return content3;
	}
	...
	private void renderBeam(int initialYOffset, ClaimBeaconBlockEntity block, PoseStack poseStack, MultiBufferSource buffer, float delta){
		...
		switch (block.getStatus()){
			...
			case ClaimBeaconBlockEntity.STATUS_STOPPING, ClaimBeaconBlockEntity.STATUS_ERRORED_STOPPING -> color1 = COLOR_STOPPING;
			...
			case ClaimBeaconBlockEntity.STATUS_CHANGING_FACTION, ClaimBeaconBlockEntity.STATUS_UNSTABLE_CHANGING_FACTION -> color1 = COLOR_EDITING;
